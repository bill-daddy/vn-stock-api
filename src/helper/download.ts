...
const createUrl = (
  isUpto: boolean,
  date: ReturnType<typeof getTodayDayMonthYear>,
) => {
...
const reNameFiles = (folder: string) => {
  ...
  const filesPath: { [key in typeof markets[number]]: string } = {
    HNX: '',
    HSX: '',
    UPCOM: '',
  };
...
const downloadDataFromCafeF = async (url: string, downloadFolder: string) => {
  ...
  try {
    ...
    fs.createReadStream(filePath).pipe(unzip.Extract({ path: downloadFolder }));
...
const downloadStockData = async (
  isUpto: boolean,
  selectDate?: ReturnType<typeof getTodayDayMonthYear>,
) => {
  ...
  const data = downloadDataFromCafeF(url, downloadFolder);
  return data;
};
